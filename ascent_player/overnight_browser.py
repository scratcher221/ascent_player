...
EVAL_EPISODES = 3
# Short-run improvement thresholds (2-minute sessions).
IMPROVE_EVAL_MEAN = 40.0
IMPROVE_EVAL_MAX = 80.0
IMPROVE_TRAIN_RECENT = 60.0
IMPROVE_LOG_MEAN = 50.0
...
def _score_improved(
    ...
    log_mean = float(log_stats.get("last50_mean", 0.0) or log_stats.get("recent_avg", 0.0))

    improved = False
    if eval_mean >= state.best_eval_mean + IMPROVE_EVAL_MEAN:
    ...
    if eval_max >= state.best_eval_max + IMPROVE_EVAL_MAX:
        ...
        improved = True
    if train_recent >= state.best_training_recent_avg + IMPROVE_TRAIN_RECENT:
        notes.append(f"train_recent +{train_recent - state.best_training_recent_avg:.0f}")
        improved = True
    if log_mean >= state.best_training_recent_avg + IMPROVE_LOG_MEAN and log_mean > 0:
        notes.append(f"log_last50_mean {log_mean:.0f}")
        improved = True

    return improved, notes


def _next_session_seconds(current: int, improved: bool, plateau_runs: int) -> int:
    if improved:
...
def _tune_hyperparams(
    ...
    if loop_hz > 0 and loop_hz < 14 and state.frame_skip < 8:
        state.frame_skip += 1
        config.training.frame_skip = state.frame_skip
    ...
    state.target_steer_gain = min(0.75, state.target_steer_gain + 0.04)
    config.reward.target_steer_gain = state.target_steer_gain
    config.reward.target_approach_gain = state.target_steer_gain * 0.83
    config.reward.direction_flip_penalty = max(
        -0.25,
        config.reward.direction_flip_penalty - 0.01,
    )
    notes.append(f"target_steer_gain→{state.target_steer_gain:.2f}")

    config.training.transfer_epsilon_restart = min(
        0.75,
        config.training.transfer_epsilon_restart + 0.08,
    )
    config.training.epsilon_end = min(0.12, config.training.epsilon_end + 0.01)
    state.epsilon_restart_count += 1
    notes.append(
        f"ε_restart→{config.training.transfer_epsilon_restart:.2f} "
        f"ε_end→{config.training.epsilon_end:.2f}"
    )
...
async def run_overnight_session(
    ...
    *,
    ...
    while state.total_elapsed_s < total_budget_s:
        ...
        journal(f"=== RUN {run_index} train {session_s}s (elapsed {state.total_elapsed_s:.0f}s) ===")
        ...
        journal(
            ...
            f"last50_mean={log_stats.get('last50_mean', 0):.0f}"
        )
        ...
        journal(
            f"EVAL done {eval_elapsed:.0f}s "
            f"mean={eval_stats.get('recent_avg', 0):.0f} "
            f"max={eval_stats.get('recent_max', 0):.0f}"
        )

        improved, improve_notes = _score_improved(state, training, log_stats, eval_stats)
        tune_notes = _tune_hyperparams(config, state, log_stats, improved)

        if improved:
            state.plateau_runs = 0
            state.best_eval_mean = max(
                state.best_eval_mean,
                float(eval_stats.get("recent_avg", 0.0)),
            )
    ...
    journal(
        ...
        f"best_eval={state.best_eval_mean:.0f} "
